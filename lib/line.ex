defmodule Line do
  ...
  defp get_neighbour_pids(n, id_map, map) when n == 0 do
    map
  end

  defp get_neighbour_pids(n, id_map, map) do
    pid = Map.get(id_map, n, :doesnt_exist)

    neighbors = [Map.get(id_map, n + 1, :doesnt_exist), Map.get(id_map, n - 1, :doesnt_exist)]

    neighbors = check_neighbors(neighbors)
    # map = Map.put(map, n, neighbors)
    map = Map.put(map, pid, neighbors)
    get_neighbour_pids(n - 1, id_map, map)
  end
  ...
  defp start_gossip(n) do
    ...
    neighbors_map = get_neighbour_pids(n, id_map, neighbors_map)
